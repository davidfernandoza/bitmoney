...
const index = async(username, firebaseToken, data) => {
	...
	const login = await db.sequelize.transaction( t => {
		...
		return db.Sessions.find({
			...
			return db.sequelize.query(Query.DataSeller, {
				bind: {
			...
			if (newData[1] != null) {
				...
				return db.Sessions.update(dataToken, {
					...
					const tokenUser = await token.createToken(newData[0].iduser, 'seller', fireToken[1][0].idsession, newData[0].id)
			...
			// Create Firebase Token
				...
				return db.Sessions.create(dataToken, {
					...
					const tokenUser = await token.createToken(newData[0].iduser, 'seller', fireToken.idsession, newData[0].id)
